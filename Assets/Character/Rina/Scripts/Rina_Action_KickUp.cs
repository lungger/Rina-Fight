...
public class Rina_Action_KickUp : ActionInterface
{
    ...
    public float attackOutDelay  = 0.2f;
    ...
    public Rina_Action_KickUp(GameCharatcer player, int ID, string Name)
    {
    ...
    private void Attack_1(ref GameObject StartReference, ref GameObject TargetReference, ref Vector3 TargetPosition, ref Quaternion Targetrotation, ref float TargetAngle, ref float RefreshedRinaRotation_Y)
    {
        //開始發射
        attacked = true;
        GameObject rinaNormalAttack_0 = Object.Instantiate(rina_Data.rinaKickUpAttack_0);
    ...
    public void ProcessAction(int currentId)
    {
        ...
        if (Mathf.Abs(Mathf.Abs(TargetAngle) - Mathf.Abs(RefreshedRinaRotation_Y)) < 30 && attackDelayTimer > attackOutDelay)
        {
            if (!attacked)
            {
                if(InputState.Now.Button_Skill1)
                    MasterScript.gameCharacterController.moveSpeed.y += 300;
                else
                    MasterScript.gameCharacterController.moveSpeed.y += 20;
